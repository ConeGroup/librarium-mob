...
import 'package:pbp_django_auth/pbp_django_auth.dart';
    ...
    Future<Book> fetchUserbyId(int userId) async {
        var url = Uri.parse('https://fazle-ilahi-c01librarium.stndar.dev/collection/get-book-by-id-json-mob/$userId/');
  ...
  // mengambil buku berdasarkan ID
    Future<Book> fetchBookById(int bookId) async {
      var url = Uri.parse('https://fazle-ilahi-c01librarium.stndar.dev/collection/get-book-by-id-json-mob/$bookId/');
      var response = await http.get(
        url,
        headers: {"Content-Type": "application/json"},
      );
  ...
  Future<List<CollectionItemModel>> fetchCollectionBook(CookieRequest request, int bookId) async {
    var response;
      try {
        response = await request.get('https://fazle-ilahi-c01librarium.stndar.dev/collection/get-collections-by-book-json-mob/$bookId/');
    ...
    Future<List<CollectionItemModel>> fetchBookCollection(CookieRequest request, int collectionId) async {
      var response;
        try {
          response = await request.get('https://fazle-ilahi-c01librarium.stndar.dev/collection/get-book-by-collection-json-mob/$collectionId/');
